...
#%%
train_X, valid_X, train_y, valid_y = train_test_split(X, y, test_size = 0.001, random_state = 144) 
...
#%%
sparse_small = hstack((X_dummies, X_brand, X_category, 
                        X_name, X_intcol, X_des)).tocsr()
X_small = sparse_small[:nrow_train]
X_small_test = sparse_small[nrow_train:]

predsL1 = modelL1.predict(X)
predsL1_test = modelL1.predict(X_test)
predsL1 =predsL1[:, np.newaxis]
predsL1_test =predsL1_test[:, np.newaxis]
X_leak = hstack((X_small, predsL1)).tocsr()
X_leak_test = hstack((X_small_test, predsL1_test)).tocsr()

modelRL = Ridge(solver="sag", fit_intercept=True, random_state=2, alpha=4, 
                tol=0.001,
                max_iter=300)
modelRL.fit(X_leak, y)
print('[{}] Finished to train ridge sag'.format(time.time() - start_time))
predsRL = modelRL.predict(X_leak)

predsRL_test = modelRL.predict(X_leak_test)

#predsR = modelRL.predict(X=X_test)
print('[{}] Finished to predict ridge sag'.format(time.time() - start_time))


submission['price'] = np.expm1(predsRL_test)
submission.to_csv("FF_LR_1.csv", index=False)
